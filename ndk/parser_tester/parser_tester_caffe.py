...
from ndk.caffe_interface import load_from_caffemodel
from running_and_test import running_and_test, running_quant
from ndk.modelpack import modelpack
from random_quantize import random_quantize, change_nonlinear
from configparser import ConfigParser, NoOptionError
import numpy as np
from ndk.optimize import merge_bn_scale_after_linear_op, merge_bn_scale_before_conv_ip, merge_layers
from ndk.layers import check_layers

config_file_name = 'parser_tester_config_caffe.ini'
prototxt_file = ''
caffemodel_file = ''
input_file = ''
golden_file = ''
...
def parser_tester_caffe(fname_prototxt, fname_caffemodel, input_fname, golden_fname, bitwidth, packing_net_for_chip, testing_float_parsing):
    out_file_path = "for_testing_time_on_chip"
    layer_list0, param_dict0 = load_from_caffemodel(fname_prototxt, fname_caffemodel)
    ...
    if testing_float_parsing:
        running_and_test(input_fname, layer_list, param_dict, golden_fname)
...
if __name__=='__main__':
    ...
    prototxt_file = conf.get('input_file_path', 'prototxt_file')
    caffemodel_file = conf.get('input_file_path', 'caffemodel_file')
    ...
    print("config get:")
    print('prototxt_file: ', prototxt_file)
    print('caffemodel_file: ', caffemodel_file)
    ...
    parser_tester_caffe(prototxt_file, caffemodel_file, input_file, golden_file, bitwidth, packing_net_for_chip, testing_float_parsing)
